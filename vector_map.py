...
GMM_filename = 'GMLRM_model/learner.pickle'
GMM = joblib.load(GMM_filename)
L = 10
l = L*1j
...
for i in range(L):
    for j in range(L):
        a = GMM.predict(state[:,i,j])[0]
        U[i,j] = a[0]
        V[i,j] = a[1]
        H[i,j] = GMM.Gaussian_Distribution(a,2,a,var)
...
cp = ax4.contourf(X, Y, H, cmap='autumn')
fig.colorbar(strm.lines)
